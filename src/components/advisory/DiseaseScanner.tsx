  ...
  return (
    <div className="p-5 rounded-2xl h-full" style={{ background: 'white', border: '1px solid rgba(167,139,250,0.15)' }}>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-4xl font-semibold" style={{ color: "#1a1a14" }}>🔬 {t('aiDiseaseScanner')}</h2>
          <p className="text-4xl mt-0.5" style={{ color: 'rgba(167,139,250,0.6)' }}>{t('uploadPhotoDesc')}</p>
        </div>
        <span className="text-4xl px-2.5 py-1 rounded-full" style={{ background: 'rgba(167,139,250,0.1)', color: '#a78bfa', border: '1px solid rgba(167,139,250,0.2)' }}>Groq AI</span>
      </div>
      ...
      {!result && !loading && (
        <>
          <input type="text" placeholder={t('cropNameOptional')} value={cropName}
            ...
            className="w-full text-4xl px-3 py-2.5 rounded-xl mb-3 outline-none"
          ...
          <div onClick={() => fileRef.current?.click()}
            ...
            {image ? (
              ...
              <div>
                <div className="w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center text-4xl"
                  style={{ background: 'rgba(167,139,250,0.1)', border: '1px solid rgba(167,139,250,0.2)' }}>📸</div>
                <p className="text-4xl font-medium mb-1" style={{ color: '#3a3a2a' }}>{t('clickOrDrop')}</p>
                <p className="text-4xl" style={{ color: '#9a9a8a' }}>{t('jpgPng')}</p>
              </div>
            )}
      ...
      {loading && (
        <div className="py-10 text-center">
          ...
          <p className="text-4xl font-medium mb-1" style={{ color: "#1a1a14" }}>{t('analyzing')}</p>
          <p className="text-4xl mb-4" style={{ color: '#8a8a7a' }}>Running Groq Llama 4 Scout Vision...</p>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-start justify-between p-4 rounded-2xl"
            ...
            <div className="flex-1">
              <p className="text-4xl mb-1" style={{ color: '#8a8a7a' }}>{t('detected')}</p>
              <p className="text-4xl font-bold mb-1" style={{ color: "#1a1a14" }}>{result.disease}</p>
              <p className="text-4xl leading-relaxed" style={{ color: '#6a6a5a' }}>{result.summary}</p>
            </div>
            <div className="text-right ml-4 flex-shrink-0">
              <div className="text-4xl font-bold" style={{ color: '#a78bfa' }}>{result.healthScore}</div>
              <div className="text-4xl mt-0.5" style={{ color: '#8a8a7a' }}>{t('healthScore')}</div>
              {result.severity && (
                <span className="text-4xl px-2 py-0.5 rounded-full mt-1.5 inline-block"
          ...
          {result.confidence > 0 && (
            <div>
              <div className="flex justify-between text-4xl mb-1.5">
          ...
          {result.treatment?.length > 0 && (
            <div className="p-4 rounded-xl" style={{ background: 'rgba(74,222,128,0.05)', border: '1px solid rgba(74,222,128,0.15)' }}>
              <p className="text-4xl font-semibold mb-3" style={{ color: "#1a1a14" }}>{t('treatmentSteps')}</p>
              <div className="space-y-2">
                {result.treatment.map((step: string, i: number) => (
                  <div key={i} className="flex items-start gap-2.5">
                    <span className="w-5 h-5 rounded-full flex items-center justify-center text-4xl flex-shrink-0 mt-0.5 font-bold"
                      style={{ background: 'rgba(74,222,128,0.15)', color: '#16a34a' }}>{i+1}</span>
                    <p className="text-4xl leading-relaxed" style={{ color: '#4a4a3a' }}>{step}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {result.nutrients?.length > 0 && (
            <div className="p-3 rounded-xl" style={{ background: 'rgba(251,191,36,0.05)', border: '1px solid rgba(251,191,36,0.15)' }}>
              <p className="text-4xl font-semibold mb-2" style={{ color: '#fbbf24' }}>{t('nutrientDeficiencies')}</p>
              <div className="flex flex-wrap gap-2">
                {result.nutrients.map((n: string, i: number) => (
                  <span key={i} className="text-4xl px-2 py-1 rounded-lg" style={{ background: 'rgba(251,191,36,0.1)', color: '#fbbf24', border: '1px solid rgba(251,191,36,0.2)' }}>{n}</span>
                ))}
              </div>
            </div>
          )}
          ...
          <button onClick={() => { setImage(null); setResult(null) }}
            className="w-full py-2.5 text-4xl rounded-xl transition-all"
