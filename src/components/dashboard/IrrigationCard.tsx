...
export default function IrrigationCard() {
  ...
  const nextIrrigation = data?.irrigationNeeded ? `${t('today')} · ${t('scheduleSoon')}` : `${t('tomorrow')} · 6:00 AM`
  ...
  return (
    <div className="bg-green-950/60 border border-green-400/15 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-green-400">{t('aiOptimized')}</span>
      </div>

      <div className="mb-4">
        <div className="flex justify-between text-xs mb-1.5">
          <span className="text-green-100/50">{t('soilMoisture')}</span>
          <span className="text-green-300 font-semibold">{moisture}%</span>
        </div>
        <div className="h-2 bg-green-400/10 rounded-full overflow-hidden">
          <div className={`h-full rounded-full ${moisture < 50 ? 'bg-yellow-400' : 'bg-blue-400'}`} style={{ width: `${moisture}%` }} />
        </div>
        <p className="text-xs text-green-100/30 mt-1">{t('optimalRange')}</p>
      </div>
      ...
      <div className={`border rounded-lg p-3 mb-3 ${data?.irrigationNeeded ? 'bg-yellow-400/8 border-yellow-400/20' : 'bg-green-400/5 border-green-400/10'}`}>
        <p className="text-xs text-green-100/40 mb-1">{t('nextIrrigationSched')}</p>
        <p className="text-sm font-semibold text-green-300">{nextIrrigation}</p>
        <p className="text-xs text-green-100/50 mt-0.5">{amount}mm · {Math.round(amount * 1.8)} min</p>
      </div>
            ...
            <div key={name} className="flex items-center justify-between text-xs">
              ...
              <div className="flex items-center gap-2">
                <div className="w-20 h-1 bg-green-400/10 rounded-full overflow-hidden">
                  <div className={`h-full rounded-full ${m < 55 ? 'bg-yellow-400' : 'bg-blue-400'}`} style={{ width: `${m}%` }} />
                </div>
                <span className={m < 55 ? 'text-yellow-400' : 'text-green-400'}>{m < 55 ? t('needed') : t('good')}</span>
              </div>
            </div>
          ))}
        </div>
