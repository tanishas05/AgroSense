      ...
      {items.map(({ icon, label, value, sub, ok, accent, barWidth }) => (
        <div key={label} className="relative overflow-hidden p-4 rounded-2xl"
          ...
          <div className="flex items-center justify-between mb-3">
            <span className="text-4xl" style={{ color: '#8a8a7a' }}>{label}</span>
            <span className="text-4xl">{icon}</span>
          </div>
          <div className="text-4xl font-bold mb-1" style={{ color: accent }}>{value}</div>
          <div className="text-4xl" style={{ color: ok ? '#16a34a' : '#d97706' }}>{sub}</div>
          {barWidth !== null && (
