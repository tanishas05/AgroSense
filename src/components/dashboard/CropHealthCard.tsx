...
export default function CropHealthCard() {
  ...
  function getHealthForCrop(crop: string) {
    const cropScans = scans.filter(s => s.crop_name?.toLowerCase() === crop.toLowerCase())
  ...
  return (
    <div className="bg-green-950/60 border border-green-400/15 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => router.push('/advisory')} className="text-[11px] text-green-400 hover:underline">
          Scan new crop
        </button>
      </div>
      ...
      {crops.length === 0 ? (
        <div className="text-center py-6">
          <p className="text-xs text-green-100/30 mb-3">No crops set yet</p>
          <button
              ...
              <div key={crop}>
                <div className="flex items-center justify-between mb-1">
                  <div>
                    ...
                    <span className="text-[10px] text-green-100/35 ml-2">Field {i + 1}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {health !== null && <span className="text-xs font-semibold text-green-300">{health}%</span>}
                  </div>
                </div>
                {health !== null ? (
                  <div className="h-1.5 bg-green-400/10 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${health >= 80 ? 'bg-green-400' : health >= 60 ? 'bg-yellow-400' : 'bg-red-400'}`}
                      style={{ width: `${health}%` }}
                    />
                  </div>
                ) : (
